    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (cooldowns.has(cooldown.user + cooldown.command)) {
                console.log("[COOLDOWN] %s is on cooldown for %s.", interaction.user.tag, interaction.commandName);
                ...
                try {
                    ...
                    console.log("[INTERACTION] %s executed the %s command in #%s.", interaction.user.tag, interaction.commandName, interaction.channel.name);
                } catch (err) {
                    console.error(err);
                }
